...
def report_on_dataset(eval_mode, all_labels, all_samples, result_dir, feature_type):
    ...
    if eval_mode:
        mapping = EVALUATION_GESTURE_MAPPING
...
def get_dataset_files_from_path(source_path, eval_mode=False):
    file_names = os.listdir(source_path)
    exclude = []
    # exclude certain gestures in eval mode
    if eval_mode:
        exclude = ["spread", "swipe_down"]
    files_to_use = []
    for file in file_names:
        for gesture in DATASET_CREATION_GESTURES:
            if gesture in file and gesture not in exclude:
...
class DataPreprocessing:
    ...
    def __init__(self,
        ...
        :param source_dir: The directory where the labelled csv data is located.
        ...
        :param data_augmentation: Determines if augmented samples should be created.
        """
        # Directories
        self.data_source_dir = f"../Data_Acquisition/{source_dir}"
        self.result_dir = f"../Data_Preprocessing/{source_dir}/{feature_type.name}_features"
        self.labelled_path = self.data_source_dir + "/video_data_labelled"

        self.eval_mode = eval_mode
        if eval_mode:
            self.result_dir = f"eval_{source_dir}/{feature_type.name}_features"
    ...
    def start(self):
        ...
        files_to_use = get_dataset_files_from_path(f"{self.labelled_path}/", eval_mode=self.eval_mode)
        files_left = files_to_use.copy()

        # self.data_for_file(results=results, file_name="rotate_right_micha.csv")
        # exit()
        ...
        if self.eval_mode:
            all_labels = self.map_labels_for_eval_mode(all_labels, mapping=EVALUATION_GESTURE_MAPPING)
    ...
    def save(self, samples: np.array, labels: np.array):
        ...
        report_on_dataset(self.eval_mode, labels, samples, self.result_dir, self.feature_type)
