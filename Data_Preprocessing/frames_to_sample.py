...
if conf_dict["mode"] == "live":
    from config_live import INTERPOLATION_RESAMPLE_RATE, FRAMES_PER_SAMPLE
else:
    from config_eval import INTERPOLATION_RESAMPLE_RATE, FRAMES_PER_SAMPLE
...
def interpolate(frames: pd.DataFrame):
    ...
    interpolated = frames.resample(INTERPOLATION_RESAMPLE_RATE).interpolate(method='linear')
